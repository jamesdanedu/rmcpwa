...
/**
 * Fetches video statistics (including view count) from YouTube API
 * @param {string} videoId - The YouTube video ID
 * @returns {Promise<Object>} Video statistics or mock data if API key not available
 */
export async function getVideoStatistics(videoId) {
  ...
  if (!apiKey) {
    console.warn('YouTube API key is not configured. Using mock statistics.');
